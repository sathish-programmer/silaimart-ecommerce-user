...
const SocialShare = ({ product, url }) => {
  ...
  const title = `Check out ${product.name} - SilaiMart`;
  const description = product.description.replace(/<[^>]*>/g, '').substring(0, 150);
  const image = product.images[0]?.url;

  const shareLinks = {
    facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}&quote=${encodeURIComponent(title)}`,
    twitter: `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(title)}&hashtags=SilaiMart,Sculpture,Handmade`,
